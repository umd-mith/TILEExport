	...
	**/
	...
	$.extend(FloatingDiv.prototype, {
		...
		setInputObject:function(o,refs){
			...
			if(!o) return;
			self._curLink=o;
			if(__v) console.log("self._curLink: "+JSON.stringify(self._curLink));
		...
		sendLabels:function(){
			...
			for(var x in lbls){
				...
				var sendref={id:self._labels[lbls[x]].id,type:"labels",jsonName:"Labels",display:lbls[x],obj:self._labels[lbls[x]]};
	...
	**/
	...
	TILE_ENGINE.prototype={
		...
		parseJSON:function(file){
			...
			if(__v) console.log("json in parseJSON: "+JSON.stringify(json));
			// obj._parseOutJSON();
			pluginControl.setJSON();
		},
				...
				if(ref1.type==ref1.jsonName){
					...
					for(var x in json[ref1.jsonName][ref1.type]){
						if(ref1.id==json[ref1.jsonName][ref1.type][x]){
							var n=$.inArray(ref2.id,json[ref1.jsonName][ref1.type][x][ref2.type]);
							if(n>=0){
								var nJson=[];
								for(var ll in json[ref1.jsonName][ref1.type][x][ref2.type]){
									if(json[ref1.jsonName][ref1.type][x][ref2.type][ll].id!=ref2.id){
										nJson.push(json[ref1.jsonName][ref1.type][x][ref2.type][ll]);
									}
								}
		...
		addButton:function(button){
			...
			for(var b in button){
				...
				if($.inArray(b,callbacks)>=0){
					// attach callback
					var callback=eval('('+button[b]+')');
					var obj=button.data;
					$("#"+button.id).live(b,{obj:obj},callback);
				}
			}
